...
def index():
    ...
    symbols = db.execute("SELECT DISTINCT symbol FROM transactions WHERE user_id = ? and action = 'BUY'", user_id)
    ...
    for symbol in symbols:
        qty_buy = db.execute("SELECT SUM(quantity) FROM transactions WHERE symbol = ? and user_id = ? and action = 'BUY'", symbol["symbol"], user_id)
        qty_sell = db.execute("SELECT SUM(quantity) FROM transactions WHERE symbol = ? and user_id = ? and action = 'SELL'", symbol["symbol"], user_id)
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO transactions(user_id, symbol, quantity, price, datetime, action) VALUES(?, ?, ?, ?, ?, 'BUY')", user_id, stock["symbol"], shares, stock["price"], time_now)
        return redirect("/")
...
def history():
    ...
    user_id = session.get("user_id")
    rows = db.execute("SELECT action, symbol, quantity, price, datetime FROM transactions WHERE user_id = ? ORDER BY datetime ASC", user_id)
    for row in rows:
...
def sell():
    ...
    if request.method == "POST":
        ...
        qty_buy = db.execute("SELECT SUM(quantity) FROM transactions WHERE symbol = ? and user_id = ? and action = 'BUY'", symbol, user_id)
        qty_sell = db.execute("SELECT SUM(quantity) FROM transactions WHERE symbol = ? and user_id = ? and action = 'SELL'", symbol, user_id)
        if not qty_sell[0]["SUM(quantity)"]:
            qty_sell[0]["SUM(quantity)"] = 0
        quantity = qty_buy[0]["SUM(quantity)"] - qty_sell[0]["SUM(quantity)"]
        if shares1 > quantity:
        ...
        db.execute("INSERT INTO transactions(user_id, symbol, quantity, price, datetime, action) VALUES(?, ?, ?, ?, ?, 'SELL')", user_id, stock["symbol"], shares1, stock["price"], time_now)
